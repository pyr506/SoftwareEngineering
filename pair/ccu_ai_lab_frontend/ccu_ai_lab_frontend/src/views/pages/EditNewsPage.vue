...
<script>
import { getNewsDetail, editNews } from "../../service/api.js";
...
export default {
  ...
  methods: {
    onSubmit() {
      editNews(this.id, this.title, this.content)
        .then(response => {
          if (response.status == 'error')
          {
            throw new Error(response.message);
          }
          alert("Save News Success");
          const backURL = "/" + window.location.hash.split("/")[1];
          this.$router.push(backURL);
        })
        .catch(error => {
          alert("Save News Error: " + error.message);
          console.log(error);
        });
    },
    isAdmin() {
  ...
  created() {
    getNewsDetail(window.location.hash.split("/")[3]).then(response => {
